...
Vec* vec_new() {
  ...
  vec.length = 0;
  vec.capacity = 0;
  return &vec; //!!local variable has freed
}

void vec_push(Vec* vec, int n) {
  if (vec->length == vec->capacity) {
    int new_capacity = vec->capacity * 2; //? 0 * 2 = 0
...
void main() {
  ...
  int* n = &vec->data[0];
  vec_push(vec, 110);
  printf("%d\n", *n); //n shouldn't have existed address.

  free(vec->data);//it doesn't make a wo
  vec_free(vec);
}
```